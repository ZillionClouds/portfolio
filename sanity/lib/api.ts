...
export const getRecommendations = async () => {
  const recommendationQuery = `*[_type == "recommendations"] | order(givenDate desc){
    recommendationBy,
    company,
  ...
  const recommendationHistory = await client.fetch<RecommendationType[]>(recommendationQuery);
